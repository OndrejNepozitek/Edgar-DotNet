...
{
    public class LevelDescriptionGrid2D<TRoom> : IMapDescription<TRoom>
    {
        ...
        public IGraph<RoomNode<TRoom>> GetGraphNew()
        {
            var graph = new UndirectedAdjacencyListGraph<RoomNode<TRoom>>();

            var counter = 0;
            var roomNodes = new List<RoomNode<TRoom>>();
            foreach (var room in roomDescriptions.Keys)
            {
                var roomNode = new RoomNode<TRoom>(counter++, room);
                graph.AddVertex(roomNode);
                roomNodes.Add(roomNode);
            }

            foreach (var passage in passages)
            {
                graph.AddEdge(roomNodes.First(x => Equals(x.Room, passage.Room1)), roomNodes.First(x => Equals(x.Room, passage.Room2)));
            }
