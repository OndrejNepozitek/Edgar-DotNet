...
using Edgar.GraphBasedGenerator.GridPseudo3D.Internal;
...
namespace Edgar.GraphBasedGenerator.GridPseudo3D
{
    ...
    /// </summary>
    public class GraphBasedGeneratorGridPseudo3D<TRoom> : IRandomInjectable, ICancellable, IObservableGenerator<LayoutGrid2D<TRoom>>
    {
        ...
        private ChainBasedGenerator<Layout<TRoom, ConfigurationGridPseudo3D<TRoom, EnergyData>>, LayoutGrid2D<TRoom>, RoomNode<TRoom>> generator;
        
        // Exists because OnPerturbed converts layouts which uses the Random instance and causes results to be different.
        private event Action<Layout<TRoom, ConfigurationGridPseudo3D<TRoom, EnergyData>>> OnPerturbedInternal;
        ...
        public GraphBasedGeneratorGridPseudo3D(LevelDescriptionGrid2D<TRoom> levelDescription, GraphBasedGeneratorConfiguration<TRoom> configuration = null)
        {
        ...
        private void SetupGenerator()
        {
            ...
            var generatorPlanner = new GeneratorPlanner<Layout<TRoom, ConfigurationGridPseudo3D<TRoom, EnergyData>>, RoomNode<TRoom>>(configuration.SimulatedAnnealingMaxBranching);
            ...
            var simpleConfigurationSpaces = new ConfigurationSpacesGridPseudo3D<ConfigurationGridPseudo3D<TRoom, EnergyData>, RoomNode<TRoom>>(levelDescriptionMapped);
            ...
            var energyUpdater = new BasicEnergyUpdater<RoomNode<TRoom>, ConfigurationGridPseudo3D<TRoom, EnergyData>>(10 * averageSize);
            var roomShapeGeometry = new FastGridPolygonGeometry<ConfigurationGridPseudo3D<TRoom, EnergyData>, RoomNode<TRoom>>();

            // Create generator constraints
            var stageOneConstraints =
                new List<INodeConstraint<ILayout<RoomNode<TRoom>, ConfigurationGridPseudo3D<TRoom, EnergyData>>, RoomNode<TRoom>, ConfigurationGridPseudo3D<TRoom, EnergyData>,
                    EnergyData>>
                {
                    new BasicConstraint<RoomNode<TRoom>, ConfigurationGridPseudo3D<TRoom, EnergyData>, EnergyData>(
                        roomShapeGeometry,
                        simpleConfigurationSpaces,
                        levelDescriptionMapped,
                        configuration.OptimizeCorridorConstraints
                    ),
                    new CorridorConstraint<RoomNode<TRoom>, ConfigurationGridPseudo3D<TRoom, EnergyData>, EnergyData>(
                        levelDescriptionMapped,
                        simpleConfigurationSpaces,
                        roomShapeGeometry
                    ),
            ...
            {
                stageOneConstraints.Add(new MinimumDistanceConstraint<RoomNode<TRoom>, ConfigurationGridPseudo3D<TRoom, EnergyData>, EnergyData>(
                    levelDescriptionMapped,
                    roomShapeGeometry,
            ...
            var constraintsEvaluator = new ConstraintsEvaluator<RoomNode<TRoom>, ConfigurationGridPseudo3D<TRoom, EnergyData>, EnergyData>(stageOneConstraints, energyUpdater);

            var roomShapesHandler = new RoomShapesHandlerGrid2D<RoomNode<TRoom>, ConfigurationGridPseudo3D<TRoom, EnergyData>>(
                intAliasMapping,
                levelDescriptionMapped,
                shapesForNodes,
            ...
            var layoutOperations = new LayoutController<Layout<TRoom, ConfigurationGridPseudo3D<TRoom, EnergyData>>, RoomNode<TRoom>, ConfigurationGridPseudo3D<TRoom, EnergyData>, RoomTemplateInstanceGrid2D, EnergyData>(averageSize, levelDescriptionMapped, constraintsEvaluator, roomShapesHandler, configuration.ThrowIfRepeatModeNotSatisfied, simpleConfigurationSpaces, roomShapeGeometry);

            var initialLayout = new Layout<TRoom, ConfigurationGridPseudo3D<TRoom, EnergyData>>(levelDescriptionMapped.GetGraph());
            var layoutConverter =
                new BasicLayoutConverterGrid2D<TRoom,
                    ConfigurationGridPseudo3D<TRoom, EnergyData>>(levelDescription, simpleConfigurationSpaces,
                    intAliasMapping);

            // Create simulated annealing evolver
            var layoutEvolver =
                    new Common.SimulatedAnnealingEvolver<Layout<TRoom, ConfigurationGridPseudo3D<TRoom, EnergyData>>, RoomNode<TRoom>,
                        ConfigurationGridPseudo3D<TRoom, EnergyData>>(layoutOperations, configuration.SimulatedAnnealingConfiguration, true);

            // Create the generator itself
            generator = new ChainBasedGenerator<Layout<TRoom, ConfigurationGridPseudo3D<TRoom, EnergyData>>, LayoutGrid2D<TRoom>, RoomNode<TRoom>>(initialLayout, generatorPlanner, chains, layoutEvolver, layoutConverter);
        ...
        private Action<Layout<TRoom, ConfigurationGridPseudo3D<TRoom, EnergyData>>> GetEarlyStoppingHandler(DateTime generatorStarted)
        {
            var iterations = 0;
