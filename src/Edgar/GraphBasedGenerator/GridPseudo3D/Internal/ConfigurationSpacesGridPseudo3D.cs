...
{
    public class ConfigurationSpacesGridPseudo3D<TConfiguration, TNode> : IConfigurationSpaces<TConfiguration, Vector2Int>, IRandomInjectable
        where TConfiguration: IConfiguration<RoomTemplateInstanceGrid2D, Vector3Int, TNode>
    {
        ...
        public ConfigurationSpacesGridPseudo3D(ILevelDescription<TNode> levelDescription, ILineIntersection<OrthogonalLineGrid2D> lineIntersection = null)
        {
        ...
        public bool HaveValidPosition(TConfiguration configuration1, TConfiguration configuration2)
        {
            ...
            var lines1 = new List<OrthogonalLineGrid2D>() {new OrthogonalLineGrid2D(configuration1.Position.Vector2Int, configuration1.Position.Vector2Int)};

            return lineIntersection.DoIntersect(space.Lines.Select(x => FastAddition(x, configuration2.Position.Vector2Int)), lines1);
        }
        ...
        private ConfigurationSpaceGridPseudo3D GetConfigurationSpaceInternal(TConfiguration configuration1, TConfiguration configuration2)
        {
            // If is over corridor
        ...
        {
            var configurationSpaces = GetConfigurationSpaceInternal(configuration1, configuration2);
            var heightDifference = configuration1.Position.Z - configuration2.Position.Z;

            if (configurationSpaces.ConfigurationSpaces.TryGetValue(heightDifference, out var configurationSpace))
            {
