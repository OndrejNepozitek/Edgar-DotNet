...
{
    public class CorridorConstraint<TNode, TConfiguration, TEnergyData> : INodeConstraint<ILayout<TNode, TConfiguration>
        ...
        where TEnergyData : ICorridorConstraintData
    {
        ...
        public CorridorConstraint(ILevelDescription<TNode> mapDescription,
            ...
            TConfiguration newConfiguration, TNode node, TConfiguration configuration, ref TEnergyData energyData)
        {
            ...
            if (AreNeighboursWithoutCorridors(perturbedNode, node))
            {
                ...
                var distanceOld = !configurationSpaces.HaveValidPosition(oldConfiguration, configuration)
                    ? ComputeDistance(configuration, oldConfiguration)
                    : 0;
                var distanceNew = !configurationSpaces.HaveValidPosition(newConfiguration, configuration)
                    ? ComputeDistance(configuration, newConfiguration)
                    : 0;

                distanceTotal = configuration.EnergyData.CorridorConstraintData.CorridorDistance +
            ...
            TNode node, ref TEnergyData energyData)
        {
            ...
            foreach (var vertex in oldLayout.Graph.Vertices)
            {
                ...
                newDistance += newNodeConfiguration.EnergyData.CorridorConstraintData.CorridorDistance -
                               nodeConfiguration.EnergyData.CorridorConstraintData.CorridorDistance;
            }
