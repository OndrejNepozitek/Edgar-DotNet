...
namespace Edgar.GraphBasedGenerator.Common.Constraints
{
    ...
    public class ConstraintsEvaluator<TNode, TConfiguration, TEnergyData>
        where TConfiguration : IEnergyConfiguration<TEnergyData>, ISmartCloneable<TConfiguration>, new()
        where TEnergyData : IEnergyData, new()
    {
        private readonly List<INodeConstraint<ILayout<TNode, TConfiguration>, TNode, TConfiguration, TEnergyData>>
            constraints;

        private readonly IEnergyUpdater<TNode, TConfiguration, TEnergyData> energyUpdater;

        public ConstraintsEvaluator(
            List<INodeConstraint<ILayout<TNode, TConfiguration>, TNode, TConfiguration, TEnergyData>> constraints,
        ...
        /// <returns></returns>
        public TEnergyData ComputeNodeEnergy(ILayout<TNode, TConfiguration> layout, TNode node,
        ...
        /// <param name="configuration"></param>
        /// <returns></returns>
        public TEnergyData UpdateNodeEnergy(ILayout<TNode, TConfiguration> layout, TNode perturbedNode,
            TConfiguration oldConfiguration, TConfiguration newConfiguration, TNode node, TConfiguration configuration)
        {
            var energyData = new TEnergyData();
            var valid = true;
            ...
            foreach (var constraint in constraints)
            {
                if (!constraint.UpdateEnergyData(layout, perturbedNode, oldConfiguration, newConfiguration, node,
                        configuration, ref energyData))
                {
                    valid = false;
                }
            }
        ...
        /// <returns></returns>
        public TEnergyData UpdateNodeEnergy(TNode node, ILayout<TNode, TConfiguration> oldLayout,
