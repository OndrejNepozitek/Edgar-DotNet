...
{
    public class BasicConstraint<TNode, TConfiguration, TEnergyData> : INodeConstraint<ILayout<TNode, TConfiguration>,
        TNode, TConfiguration, TEnergyData>
        ...
        where TEnergyData : IBasicConstraintData
    {
        ...
        public BasicConstraint(IRoomShapeGeometry<TConfiguration> roomShapeGeometry,
            IConfigurationSpaces<TConfiguration> configurationSpaces, ILevelDescription<TNode> mapDescription,
            bool optimizeCorridors)
        {
            ...
            TConfiguration newConfiguration, TNode node, TConfiguration configuration, ref TEnergyData energyData)
        {
            ...
            if (!isCorridor && AreNeighbours(layout, perturbedNode, node))
            {
                ...
                var distanceOld =
                    overlapOld == 0 && !configurationSpaces.HaveValidPosition(oldConfiguration, configuration)
                        ? ComputeDistance(configuration, oldConfiguration)
                        : 0;
                var distanceNew =
                    overlapNew == 0 && !configurationSpaces.HaveValidPosition(newConfiguration, configuration)
                        ? ComputeDistance(configuration, newConfiguration)
                        : 0;
            ...
            TNode node, ref TEnergyData energyData)
        {
            ...
            foreach (var vertex in oldLayout.Graph.Vertices)
            {
                ...
                newOverlap += newNodeConfiguration.EnergyData.BasicConstraintData.Overlap -
                              nodeConfiguration.EnergyData.BasicConstraintData.Overlap;
                newDistance += newNodeConfiguration.EnergyData.BasicConstraintData.MoveDistance -
                               nodeConfiguration.EnergyData.BasicConstraintData.MoveDistance;
            }
