...
namespace Edgar.GraphBasedGenerator.Grid2DSimplified
{
    public class LevelBuilder<TRoom> : IRandomInjectable
    {
        ...
        public bool TryConnectRoomToNeighbors(SimpleLayout<TRoom, SimpleConfiguration<TRoom>> layout, TRoom room, RoomTemplateInstanceGrid2D roomTemplate, List<ConnectionTarget<TRoom>> neighborConfigurations, int maximumAttempts = -1)
        {
            ...
            using (var transaction = layout.BeginChanges())
            {
                for (int i = 0; i < maximumAttempts; i++)
                {
                    ...
                    layout.SetConfiguration(room, new SimpleConfiguration<TRoom>()
                    {
                        Room = room,
                        Position = position,
                        RoomShape = roomTemplate,
                    });
        ...
        public bool TryConnectRoomToNeighbors(SimpleLayout<TRoom, SimpleConfiguration<TRoom>> layout, TRoom room, RoomTemplateInstanceGrid2D roomTemplate, List<RoomTemplateInstanceGrid2D> corridorRoomTemplates, int maximumAttempts = -1)
        {
        ...
        public bool TryAddCorridorRoom(SimpleLayout<TRoom, SimpleConfiguration<TRoom>> layout, TRoom room, RoomTemplateInstanceGrid2D roomTemplate, int maximumAttempts = -1)
        {
            // TODO: uncomment
            //if (!layout.IsCorridor(room))
            //{
        ...
        public bool TryAddCorridorRoom(SimpleLayout<TRoom, SimpleConfiguration<TRoom>> layout, TRoom room, List<RoomTemplateInstanceGrid2D> roomTemplates, int maximumAttempts = -1)
        {
            // TODO: uncomment
            //if (!layout.IsCorridor(room))
            //{
        ...
        public bool TryAddCorridors(SimpleLayout<TRoom, SimpleConfiguration<TRoom>> layout, TRoom room, List<RoomTemplateInstanceGrid2D> corridorRoomTemplates, int maximumAttempts = -1)
        {
            if (layout.IsCorridor(room))
            {
                throw new ArgumentException($"The room {room} is a corridor.", nameof(room));
