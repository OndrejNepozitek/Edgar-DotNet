...
namespace Edgar.GraphBasedGenerator.Grid2D.Internal
{
    ...
    public class BasicLayoutConverterGrid2D<TNode, TConfiguration> :
        ILayoutConverter<ILayout<RoomNode<TNode>, TConfiguration>, LayoutGrid2D<TNode>>, IRandomInjectable
        ...
        public LayoutGrid2D<TNode> Convert(ILayout<RoomNode<TNode>, TConfiguration> layout, bool addDoors)
        {
            ...
            foreach (var vertexAlias in layout.Graph.Vertices)
            {
                if (layout.GetConfiguration(vertexAlias, out var configuration))
                {
                    ...
                    var room = new LayoutRoomGrid2D<TNode>(vertex, transformedShape, configuration.Position - offset,
                        MapDescription.GetRoomDescription(vertexAlias.Room).IsCorridor,
                        roomTemplateInstance.RoomTemplate, MapDescription.GetRoomDescription(vertexAlias.Room),
                        transformation);
                    rooms.Add(room);
                ...
                foreach (var vertexAlias in layout.Graph.Vertices)
                {
                    ...
                    if (layout.GetConfiguration(vertexAlias, out var configuration))
                    {
                        ...
                        foreach (var neighbourAlias in neighbours)
                        {
                            ...
                            if (layout.GetConfiguration(neighbourAlias, out var neighbourConfiguration) &&
                                !generatedDoors.Contains(Tuple.Create(neighbour, vertex)))
                            {
                                ...
                                var otherDoor = new OrthogonalLineGrid2D(
                                    ...
                                    otherDoorLine.Line.From +
                                    otherDoorLine.Length * otherDoorLine.Line.GetDirectionVector(),
        ...
        private List<DoorLinePair> GetDoors(Vector2Int position, ConfigurationSpaceGrid2D configurationSpace)
        {
            ...
            foreach (var reverseDoorsInfo in configurationSpace.ReverseDoors)
            {
                ...
                var numberOfPositions =
                    Math.Min(Math.Min(offset, Math.Min(index, line.Length - index)), doorLine.Line.Length) + 1;
                ...
                for (var i = 0; i < numberOfPositions; i++)
                {
                    ...
                    var singleDoor = new DoorLineGrid2D(singleDoorLine, doorLine.Length, doorLine.DoorSocket,
                        doorLine.Type);
                    ...
                    if (doorLine.Length != 0)
                    {
                        ...
                        otherSingleDoorLine = new OrthogonalLineGrid2D(singleDoorLine.To + moveOffset,
                            singleDoorLine.From + moveOffset, otherSingleDoorLine.GetDirection());
                    }
