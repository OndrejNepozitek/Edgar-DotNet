...
namespace Edgar.Legacy.GeneralAlgorithms.Algorithms.Common
{
    public class OrthogonalLineIntersection : ILineIntersection<OrthogonalLineGrid2D>
    {
        ...
        /// <returns></returns>
        public List<OrthogonalLineGrid2D> GetIntersections(List<OrthogonalLineGrid2D> lines1,
            List<OrthogonalLineGrid2D> lines2)
        {
            return GetIntersectionsLazy(lines1, lines2).ToList();
        }

        public List<OrthogonalLineGrid2D> GetIntersections(ImmutableArray<OrthogonalLineGrid2D> lines1,
            List<OrthogonalLineGrid2D> lines2, Vector2Int offset)
        {
        ...
        private IEnumerable<OrthogonalLineGrid2D> GetIntersectionsLazy(IEnumerable<OrthogonalLineGrid2D> lines1,
            IReadOnlyCollection<OrthogonalLineGrid2D> lines2)
        {
            foreach (var line1 in lines1)
            {
                foreach (var line2 in lines2)
        ...
        public bool TryGetIntersection(OrthogonalLineGrid2D line1, OrthogonalLineGrid2D line2,
            out OrthogonalLineGrid2D intersection)
        {
            ...
            if (horizontal1 && horizontal2)
            {
                ...
                if (x1 <= x2)
                {
                    intersection = new OrthogonalLineGrid2D(new Vector2Int(x1, line1.From.Y),
                        new Vector2Int(x2, line1.From.Y));
                    return true;
                }
            ...
            else if (!horizontal1 && !horizontal2)
            {
                ...
                if (y1 <= y2)
                {
                    intersection = new OrthogonalLineGrid2D(new Vector2Int(line1.From.X, y1),
                        new Vector2Int(line1.From.X, y2));
                    return true;
                }
            ...
            else
            {
                ...
                {
                    intersection = new OrthogonalLineGrid2D(new Vector2Int(vline.From.X, hline.From.Y),
                        new Vector2Int(vline.From.X, hline.From.Y));
                    return true;
                }
