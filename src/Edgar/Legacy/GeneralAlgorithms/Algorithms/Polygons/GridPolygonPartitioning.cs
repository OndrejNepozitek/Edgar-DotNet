...
namespace Edgar.Legacy.GeneralAlgorithms.Algorithms.Polygons
{
    ...
    public class GridPolygonPartitioning : IPolygonPartitioning
    {
        ...
        private List<Tuple<Segment, Segment>> FindCrossings(List<Segment> horizontalDiagonals,
            List<Segment> verticalDiagonals)
        {
        ...
        private class Segment : IRangeProvider<int>
        {
            ...
            public Segment(Vertex from, Vertex to, bool horizontal)
            {
                ...
                if (horizontal)
                {
                    Range = new Range<int>(System.Math.Min(from.Point.X, to.Point.X),
                        System.Math.Max(from.Point.X, to.Point.X));
                }
                else
                {
                    Range = new Range<int>(System.Math.Min(from.Point.Y, to.Point.Y),
                        System.Math.Max(from.Point.Y, to.Point.Y));
                }
            }
