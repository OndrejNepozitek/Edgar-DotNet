...
namespace Edgar.Legacy.Core.LayoutConverters
{
    ...
    public class BasicLayoutConverter<TLayout, TNode, TConfiguration> : ILayoutConverter<TLayout, MapLayout<TNode>>,
        IRandomInjectable
        ...
        where TConfiguration : IConfiguration<IntAlias<PolygonGrid2D>, int>
    {
        ...
        protected readonly IConfigurationSpaces<int, IntAlias<PolygonGrid2D>, TConfiguration, ConfigurationSpace>
            ConfigurationSpaces;
        ...
        public MapLayout<TNode> Convert(TLayout layout, bool addDoors)
        {
            ...
            foreach (var vertexAlias in layout.Graph.Vertices)
            {
                if (layout.GetConfiguration(vertexAlias, out var configuration))
                {
                    ...
                    var room = new Room<TNode>(vertex, transformedShape, configuration.Position - offset,
                        MapDescription.GetRoomDescription(vertexAlias) is CorridorRoomDescription,
                        roomTemplateInstance.RoomTemplate, MapDescription.GetRoomDescription(vertexAlias),
                        transformation, roomTemplateInstance.Transformations, roomTemplateInstance);
                    rooms.Add(room);
                        ...
                        foreach (var neighbourAlias in neighbours)
                        {
                                ...
                                !generatedDoors.Contains(Tuple.Create(neighbour, vertex)))
                            {
                                ...
                                roomsDict[vertex].Doors.Add(new LayoutDoorGrid2D<TNode>(vertex, neighbour, randomChoice,
                                    null, DoorType.Undirected));
                                roomsDict[neighbour].Doors.Add(new LayoutDoorGrid2D<TNode>(neighbour, vertex,
                                    randomChoice, null, DoorType.Undirected));
