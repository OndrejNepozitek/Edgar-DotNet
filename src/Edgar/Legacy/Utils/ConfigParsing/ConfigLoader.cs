...
namespace Edgar.Legacy.Utils.ConfigParsing
{
    ...
    public class ConfigLoader
    {
        ...
        private void LoadPassages(MapDescription<int> mapDescription, MapDescriptionModel mapDescriptionModel,
            Dictionary<string, RoomDescriptionsSetModel> roomDescriptionsSets)
        {
        ...
        private CorridorRoomDescription GetCorridors(MapDescriptionModel mapDescriptionModel,
            Dictionary<string, RoomDescriptionsSetModel> roomDescriptionsSets)
        {
            ...
            if (enable && (corridors.CorridorShapes == null || corridors.CorridorShapes.Count == 0))
                throw new InvalidOperationException(
                    "There must be at least one shape for corridors if they are enabled.");

            var roomTemplates = GetRoomTemplates(corridors.CorridorShapes, roomDescriptionsSets,
                mapDescriptionModel.CustomRoomDescriptionsSet);
            var roomDescription = new CorridorRoomDescription(roomTemplates);

            return roomDescription;
        }

        private void LoadRooms(MapDescription<int> mapDescription, MapDescriptionModel mapDescriptionModel,
            Dictionary<string, RoomDescriptionsSetModel> roomDescriptionsSets)
        {
            ...
            if (mapDescriptionModel.RoomsRange.To - mapDescriptionModel.RoomsRange.From <= 0)
                throw new InvalidOperationException(
                    "There must be at least one roon in the room range. 'To' must be greater than 'From'.");
            ...
            if (mapDescriptionModel.Rooms != null)
            {
                foreach (var pair in mapDescriptionModel.Rooms)
                {
                    ...
                    {
                        var roomTemplates = GetRoomTemplates(roomModel.RoomShapes, roomDescriptionsSets,
                            mapDescriptionModel.CustomRoomDescriptionsSet);
            ...
            if (notUsedRooms.Count != 0)
            {
                ...
                {
                    var roomTemplates = GetRoomTemplates(mapDescriptionModel.DefaultRoomShapes, roomDescriptionsSets,
                        mapDescriptionModel.CustomRoomDescriptionsSet);
                    var roomDescription = new BasicRoomDescription(roomTemplates);
                ...
                {
                    throw new InvalidOperationException(
                        $"Default room shapes must be provided if there are rooms that do not have their own room shapes assigned. Rooms with missing room shapes: {string.Join(", ", notUsedRooms)}");
                }
            }
        }
        ...
        private List<RoomTemplate> GetRoomTemplates(List<RoomShapesModel> roomShapesModels,
            Dictionary<string, RoomDescriptionsSetModel> roomDescriptionsSets,
            RoomDescriptionsSetModel customRoomDescriptionsSet)
        {
            var roomTemplates = new List<RoomTemplate>();

            foreach (var roomShapesModel in roomShapesModels)
            {
                roomTemplates.AddRange(GetRoomTemplates(roomShapesModel, roomDescriptionsSets,
        ...
        private List<RoomTemplate> GetRoomTemplates(RoomShapesModel roomShapesModel,
            Dictionary<string, RoomDescriptionsSetModel> roomDescriptionsSets,
            RoomDescriptionsSetModel customRoomDescriptionsSet)
        {
            ...
            if (!string.IsNullOrEmpty(roomShapesModel.RoomDescriptionName) &&
                !setModel.RoomDescriptions.ContainsKey(roomShapesModel.RoomDescriptionName))
                throw new ParsingException(
                    $"Room description with name \"{roomShapesModel.RoomDescriptionName}\" was not found in the set \"{setName}\"");

            var roomModels = GetFilledRoomModels(setModel, roomShapesModel.RoomDescriptionName, setName);
            var roomTemplates = roomModels
                .Select(x => ConvertRoomModelToRoomTemplate(x, scale, transformations, setName)).ToList();

            return roomTemplates;
        }

        private RoomTemplate ConvertRoomModelToRoomTemplate(RoomDescriptionModel model, Vector2Int? scale,
            List<TransformationGrid2D> transformations, string setName)
        {
            ...
            var roomTemplate = new RoomTemplate(new PolygonGrid2D(model.Shape).Scale(scale ?? new Vector2Int(1, 1)),
                GetDoorMode(model.DoorMode), transformations, model.RepeatMode ?? RoomTemplateRepeatMode.AllowRepeat);
        ...
        private List<RoomDescriptionModel> GetFilledRoomModels(RoomDescriptionsSetModel setModel,
            string roomDescriptionName, string setName)
        {
            ...
            foreach (var pair in setModel.RoomDescriptions)
            {
                ...
                if (roomModel.Shape == null)
                {
                    if (defaultModel.Shape == null)
                        throw new ParsingException(
                            $"Neither shape nor default shape are set. Room {name}, set {setName}");
                ...
                if (roomModel.DoorMode == null)
                {
                    if (defaultModel.DoorMode == null)
                        throw new ParsingException(
                            $"Neither door mode nor default door mode are set. Room {name}, set {setName}");
        ...
        private RoomDescriptionsSetModel GetSetModel(string name,
            Dictionary<string, RoomDescriptionsSetModel> roomDescriptionsSets,
            RoomDescriptionsSetModel customRoomDescriptionsSet)
        {
