...
namespace Edgar.Legacy.Utils.MapDrawing
{
    ...
    public abstract class AbstractLayoutDrawer<TNode>
    {
        ...
        protected void DrawLayout(MapLayout<TNode> layout, int width, int height, bool withNames,
        ...
        /// <param name="withNames">Whether names should be displayed</param>
        /// <param name="fixedFontSize"></param>
        protected void DrawLayout(MapLayout<TNode> layout, float scale, Vector2Int offset, bool withNames,
            int? fixedFontSize = null)
        {
            var polygons = layout.Rooms.Select(x => x.Shape + x.Position).ToList();
            var rooms = layout.Rooms.ToList();
            var minWidth = layout.Rooms.Where(x => !x.IsCorridor).Select(x => x.Shape + x.Position)
