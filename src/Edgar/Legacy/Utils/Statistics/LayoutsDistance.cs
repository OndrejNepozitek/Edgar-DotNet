...
namespace Edgar.Legacy.Utils.Statistics
{
    public static class LayoutsDistance
    {
        ...
        {
            var nodeToRoom1 = layout1.Rooms.Where(x => !x.IsCorridor)
                .ToDictionary(x => x.Node, x => x.Shape + x.Position);
            var nodeToRoom2 = layout2.Rooms.Where(x => !x.IsCorridor)
                .ToDictionary(x => x.Node, x => x.Shape + x.Position);
        ...
        public static double PositionAndShapeDistance<TNode>(MapLayout<TNode> layout1, MapLayout<TNode> layout2,
            double differentShapePenalty)
        {
            var nodeToRoomShape1 = layout1.Rooms.Where(x => !x.IsCorridor)
                .ToDictionary(x => x.Node, x => (x.Shape, x.RoomTemplate));
            var nodeToRoomShape2 = layout2.Rooms.Where(x => !x.IsCorridor)
                .ToDictionary(x => x.Node, x => (x.Shape, x.RoomTemplate));

            var differentShapeCounter = 0;
            foreach (var node in nodeToRoomShape1.Keys)
            {
                if (!Equals(nodeToRoomShape1[node].Shape, nodeToRoomShape2[node].Shape) ||
                    !Equals(nodeToRoomShape1[node].RoomTemplate, nodeToRoomShape2[node].RoomTemplate))
                {
