...
{
    public class
        DungeonGeneratorEvolution<TNode> : ConfigurationEvolution<DungeonGeneratorConfiguration<TNode>,
            Individual<TNode>>
    {
        private readonly IMapDescription<TNode> mapDescription;

        private readonly BenchmarkRunnerLegacy<IMapDescription<TNode>> benchmarkRunner =
            new BenchmarkRunnerLegacy<IMapDescription<TNode>>();
        ...
        private readonly LayoutsClustering<MapLayout<TNode>> layoutsClustering =
            new LayoutsClustering<MapLayout<TNode>>();
        ...
        {
            return
                $"DungeonGeneratorEvolutions/{new DateTimeOffset(DateTime.UtcNow).ToUnixTimeSeconds()}_{generatorInput.Name}/";
        }
        ...
        protected override Individual<TNode> EvaluateIndividual(Individual<TNode> individual)
        {
            ...
            var scenario = new BenchmarkScenario<IMapDescription<TNode>>("SimulatedAnnealingParameters",
                input =>
                {
                    ...
                    {
                        individual.Configuration.EarlyStopIfIterationsExceeded = Math.Min(
                            50 * (int) individual.Parent.Iterations,
                            InitialIndividual.Configuration.EarlyStopIfIterationsExceeded ?? int.MaxValue);
                    }
                    else
                    {
                        // TODO: how to do this?
                    ...
                    var generatorRunner = new LambdaGeneratorRunner(() =>
                    {
                        ...
                        var generatorRun = new GeneratorRun<AdditionalRunData<TNode>>(layout != null,
                            layoutGenerator.TimeTotal, layoutGenerator.IterationsCount, additionalData);
                    ...
                    if (individual.Parent != null)
                    {
                        return new EarlyStoppingGeneratorRunner(generatorRunner, individual.Parent.Iterations,
                            (successful, time, iterations) =>
                                new GeneratorRun<AdditionalRunData<TNode>>(successful, time, iterations, null));
                    }
                    else
                    {
            ...
            var scenarioResult = benchmarkRunner.Run(scenario,
                new List<GeneratorInput<IMapDescription<TNode>>>()
            ...
            individual.Fitness =
                Options.FitnessType == FitnessType.Iterations ? individual.Iterations : individual.Time;
            ...
            var positionOnlyClusters = layoutsClustering.GetClusters(layoutsForClustering,
                LayoutsDistance.PositionOnlyDistance, averageRoomTemplateSize);
            var positionAndShapeClusters = layoutsClustering.GetClusters(layoutsForClustering,
                (x1, x2) => LayoutsDistance.PositionAndShapeDistance(x1, x2, averageRoomTemplateSize),
                averageRoomTemplateSize);

            var summary = "";

            var fitnessDifferenceParent = individual.Parent != null
                ? StatisticsUtils.DifferenceToReference(individual, individual.Parent, x => x.Fitness)
                : 0;
            ...
            var iterationsDifferenceParent = individual.Parent != null
                ? StatisticsUtils.DifferenceToReference(individual, individual.Parent, x => x.Iterations)
                : 0;
            ...
            var timeDifferenceParent = individual.Parent != null
                ? StatisticsUtils.DifferenceToReference(individual, individual.Parent, x => x.Time)
                : 0;
            var timeDifferenceTotal =
                StatisticsUtils.DifferenceToReference(individual, InitialIndividual, x => x.Time);

            summary +=
                $"  fitness {individual.Fitness:F}, {(fitnessDifferenceParent > 0 ? "+" : "")}{fitnessDifferenceParent:F}%, {(fitnessDifferenceTotal > 0 ? "+" : "")}{fitnessDifferenceTotal:F}% total  \n";
            summary +=
                $"  iterations {individual.Iterations:F}, {(iterationsDifferenceParent > 0 ? "+" : "")}{iterationsDifferenceParent:F}%, {(iterationsDifferenceTotal > 0 ? "+" : "")}{iterationsDifferenceTotal:F}% total  \n";
            summary +=
                $"  time {individual.Time:F}, {(timeDifferenceParent > 0 ? "+" : "")}{timeDifferenceParent:F}%, {(timeDifferenceTotal > 0 ? "+" : "")}{timeDifferenceTotal:F}% total  \n";
            summary +=
                $"  entropy {roomTemplatesEntropy:F}, clusters {positionOnlyClusters.Count}/{positionAndShapeClusters.Count} \n";
            ...
            resultSaver.SaveResultDefaultLocation(scenarioResult, $"{individual.Id}_benchmarkResults", ResultsDirectory,
                withDatetime: false);
        ...
        protected override Individual<TNode> CreateInitialIndividual(int id,
            DungeonGeneratorConfiguration<TNode> configuration)
        {
            return new Individual<TNode>(id, configuration);
        }

        protected override Individual<TNode> CreateIndividual(int id, Individual<TNode> parent,
            IMutation<DungeonGeneratorConfiguration<TNode>> mutation)
        {
