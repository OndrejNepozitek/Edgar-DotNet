...
namespace Edgar.GraphBasedGenerator.Grid2D.Drawing
{
    ...
    public abstract class AbstractLayoutDrawer<TNode>
    {
        ...
        protected void DrawLayout(LayoutGrid2D<TNode> layout, int width, int height, bool withNames,
            int? fixedFontSize = null, float borderSize = 0.2f)
        {
        ...
        /// <param name="withNames">Whether names should be displayed</param>
        /// <param name="fixedFontSize"></param>
        protected void DrawLayout(LayoutGrid2D<TNode> layout, float scale, Vector2Int offset, bool withNames,
            int? fixedFontSize = null)
        {
            var polygons = layout.Rooms.Select(x => x.Outline + x.Position).ToList();
            var rooms = layout.Rooms.ToList();
            var minWidth = layout.Rooms.Where(x => !x.IsCorridor).Select(x => x.Outline + x.Position)
                .Min(x => x.BoundingRectangle.Width);
            ...
            for (var i = 0; i < rooms.Count; i++)
            {
                ...
                var transformedPoints = polygons[i].GetPoints().Select(point => TransformPoint(point, scale, offset))
                    .ToList();

                if (transformedPoints.All(x => x == new Vector2Int(0, 0)))
                {
                    throw new InvalidOperationException(
                        "One of the polygons could not be drawn because the canvas size is too small.");
                }
        ...
        /// <returns></returns>
        public float GetScale(int minx, int miny, int maxx, int maxy, int expectedWidth, int expectedHeight,
        ...
        protected virtual void DrawRoomBefore(PolygonGrid2D polygon, List<Tuple<Vector2Int, bool>> outline,
            float penWidth)
        {
        }
