...
namespace Edgar.SandboxEvolutionRunner.Benchmarks.GraphBasedGenerator.Benchmarks
{
    public class IndependentClustersBenchmark : Benchmark
    {
            ...
            int maxDegree = 4, int nonTreeEdges = 0)
        {
            ...
            // var highLevelStructure = GetRandomGraph(numberOfClusters, 3, new Random(0), nonTreeEdges);
            var randomGraph = GetRandomGraph(verticesPerCluster, maxDegree, random, nonTreeEdges);
            var clusters =
        ...
        private BenchmarkScenario<string> GetScenario(int numberOfClusters, int verticesPerCluster, int nonTreeEdges)
        {
            ...
            {
                namedGraphs.Add(GetGraph(i + 1, verticesPerCluster, new Random(1), 4, nonTreeEdges));
            }
